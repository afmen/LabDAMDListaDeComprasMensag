...
class JsonDatabase {
    ...
    async update(id, updates) {
        try {
            ...
            // Aplicar atualizações inteligentes (suporte a dot notation)
            const currentDoc = documents[index];
            const updatedFields = this.applyUpdates(currentDoc, updates);
    ...
    async updateIndex(document) {
        try {
            const index = await fs.readJson(this.indexPath).catch(() => ({}));
            index[document.id] = {
                id: document.id,
    ...
    // Lógica de Filtro Aprimorada (com suporte a $or e operadores)
    matchesFilter(document, filter) {
        // Suporte a operador lógico $or na raiz
        if (filter.$or && Array.isArray(filter.$or)) {
            return filter.$or.some(subFilter => this.matchesFilter(document, subFilter));
        }

        return Object.entries(filter).every(([key, value]) => {
            // Ignorar chaves especiais processadas acima
            ...
            if (typeof value === 'object' && value !== null) {
                ...
                if (value.$in) {
                    return Array.isArray(value.$in) && value.$in.includes(docValue);
                }
                if (value.$gt) {
    ...
    // Aplica updates com suporte a nested keys ('profile.theme')
    applyUpdates(doc, updates) {
        const updatedDoc = { ...doc }; // Clone raso inicial

        Object.keys(updates).forEach(key => {
            if (key.includes('.')) {
                // Update profundo (dot notation)
